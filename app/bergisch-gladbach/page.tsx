...
export default function BergischGladbachPage() {
  ...
  return <main className="city-page">
    ...
    <section className="city-districts section region-links"><div className="section-head"><div><p className="eyebrow">Region &amp; Umland</p><h2>Standorte rund um Bergisch Gladbach.</h2></div><p>Eigene Ortsprofile mit kommunalen Quellen und Marktzahlen des jeweils zuständigen Gutachterausschusses.</p></div><div className="city-district-grid">{[["Bechen","bechen"],["Engelskirchen","engelskirchen"],["Königsforst","koenigsforst"],["Kürten","kuerten"],["Lindlar","lindlar"],["Odenthal","odenthal"],["Overath","overath"]].map(([name,slug])=><a href={`/${slug}/`} key={slug}><div><strong>{name}</strong><small>Standortprofil &amp; Marktdaten</small></div><b>→</b></a>)}</div></section>
