...
export default function BergischGladbachPage() {
  ...
  return <main className="city-page">
    ...
    <section className="city-districts section region-links"><div className="section-head"><div><p className="eyebrow">Region &amp; Umland</p><h2>Standorte rund um Bergisch Gladbach.</h2></div><p>Eigene Ortsprofile mit kommunalen Quellen und Marktzahlen des jeweils zuständigen Gutachterausschusses.</p></div><div className="city-district-grid">{[["Overath","overath"],["Odenthal","odenthal"],["Königsforst","koenigsforst"],["Kürten","kuerten"],["Bechen","bechen"],["Lindlar","lindlar"],["Engelskirchen","engelskirchen"]].map(([name,slug],index)=><a href={`/${slug}/`} key={slug}><span>{String(index+1).padStart(2,"0")}</span><div><strong>{name}</strong><small>Standortprofil &amp; Marktdaten</small></div><b>→</b></a>)}</div></section>
