...
export default async function DistrictPage({params}:{params:Promise<{slug:string}>}){
  ...
  return <main className="district-page">
    ...
    <section className="district-hero" style={image?{backgroundImage:`linear-gradient(90deg,rgba(0,0,0,.72),rgba(0,0,0,.1)),url(${image.src})`}:undefined}><div><p className="eyebrow light">Makler {district.name}</p><h1>Immobilienmakler {district.name}</h1><p>Immobilien verkaufen und bewerten – mit persönlicher Beratung durch den Standort Bergisch Gladbach.</p><a className="button gold" href="/immobilienbewertung/">Immobilienbewertung {district.name}</a></div>{image&&<a className="district-photo-credit" href={image.source} target="_blank" rel="noreferrer">Foto: {image.author} · Wikimedia Commons · {image.license} ↗</a>}</section>
    ...
    <section className="district-contact section" id="kontakt"><div><p className="eyebrow light">Kostenlose Erstberatung</p><h2>Immobilienbewertung in {district.name}</h2><p>Unverbindliche Anfrage an unser Team in Bergisch Gladbach.</p></div><div><a className="button gold" href={`/immobilienbewertung/?ort=${encodeURIComponent(`Bergisch Gladbach-${district.name}`)}#bewertung`}>Bewertung anfragen</a><a href="tel:+4922049147881">+49 2204 914 7881</a></div></section>
    <footer><div className="footer-brand"><span className="brand-mark">S<span>&</span>H</span><div><strong>Stark & Hoffmann Immobilien</strong><small>Evernest Lizenzpartner Bergisch Gladbach</small></div></div><div><h4>Kontakt</h4><p>Schloßstraße 41<br/>51429 Bergisch Gladbach</p><a href="tel:+4922049147881">+49 2204 914 7881</a><a href="/bergisch-gladbach/">Bergisch Gladbach</a></div><div><h4>Quellen</h4><a href="https://www.boris.nrw.de/">BORIS-NRW</a><a href="https://www.gars.nrw/stadt-gl/produkte-gl/bodenrichtwerte-gl">Gutachterausschuss</a></div><div><h4>Rechtliches</h4><a href="/impressum/">Impressum</a><a href="/agb/">AGB</a><a href="/datenschutz/">Datenschutz</a><p>Bodenrichtwerte sind Orientierungswerte und keine Verkehrswerte.</p></div></footer>
  </main>;
}
