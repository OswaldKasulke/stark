...
import { soldByPlace } from "./verkauft";
...
export default function StandortPage({ slug }: { slug: string }) {
  ...
  return <main className="city-page location-page">
    ...
    <section className="city-hero location-hero" style={image?{position:"relative",backgroundImage:`linear-gradient(90deg,rgba(0,0,0,.92),rgba(0,0,0,.38)),url(${image.src})`,backgroundPosition:"center",backgroundSize:"cover",backgroundRepeat:"no-repeat"}:undefined}><div><p className="eyebrow light">{place.label} · {place.region}</p><h1>Immobilienmakler {place.name}</h1><p>{place.intro} Wir unterstützen Eigentümer dabei, Haus, Wohnung oder Grundstück marktgerecht zu bewerten und zu verkaufen.</p><div className="hero-actions"><a className="button gold" href="/immobilienbewertung/">Immobilie bewerten</a><a className="text-link light" href="#profil">Ortsprofil <span>↓</span></a></div></div>{image&&<a className="district-photo-credit" href={image.source} target="_blank" rel="noreferrer">Quelle: Wikipedia ↗</a>}</section>
    ...
    <SoldReferences place={place.name} items={soldByPlace[slug]} />
