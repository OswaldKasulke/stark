...
const AUSSERHALB_GEBIET="Bergisch Gladbachs";
...
const apartmentPrice=(district:string,year:number,units:number):[number,string]=>{
  ...
  // Der Grundstuecksmarktbericht fuehrt Refrath und Frankenforst zusammen
  // (Kap. 6.1.1 "Stadtteile Refrath, Frankenforst" und Kap. 5.1.2
  // "Stadtteile Refrath / Frankenforst"). Frankenforst fiel bisher in den
  // stadtweiten Auffangwert und wurde dadurch zu hoch angesetzt.
  if(district==="Refrath"||district==="Frankenforst"){
    const label=district==="Frankenforst"?"Refrath / Frankenforst":"Refrath";
    if(year<=1975)return[2960,`${label}, Baujahr 1960–1975`];
    if(year<=1989)return[3240,`${label}, Baujahr 1976–1989`];
    if(year<=2000)return[3680,`${label}, Baujahr 1990–2000`];
    return[4750,`${label}, Baujahr 2001–2015`];
  }
